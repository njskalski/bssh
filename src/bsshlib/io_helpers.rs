...
pub fn write_name_list(stream : &mut Write, names : &Vec<&str>) -> Result<(), Error> {
...
pub fn read_name_list(stream : &mut Read, max_length : Option<u32>) -> Result<Vec<String>, Error> {
    ...
    for substr in payload[1..payload.len()-1].split(|c| *c == b',') {
        ...
        match from_utf8(&substr_body) {
            ...
            _ => return Err(Error::new(ErrorKind::InvalidData, errors::BSSH_ERR_MALFORMED_NAME_LIST)) //TODO more descriptive error?
        }
    }
...
mod tests {
    ...
    fn read_name_lists_works() {
        let hello_string : Vec<u8> = [ [0 as u8, 0, 0, 17].to_vec(), b"(\"hello\",\"world\")".to_vec()].concat();
        let mut mrs = MockReadStream::new(hello_string);
        assert_eq!(read_name_list(&mut mrs, None).unwrap(), vec!("hello", "world"));
    }
    ...
    fn write_name_list_works() {
        let hello_string : Vec<u8> = [ [0 as u8, 0, 0, 17].to_vec(), b"(\"hello\",\"world\")".to_vec()].concat();
        let mut mws = MockWriteStream::new();
        write_name_list(&mut mws, &vec!("hello", "world")).unwrap();
        assert_eq!(mws.output, hello_string);
    }
